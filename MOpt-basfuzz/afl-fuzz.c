...
break;
  ...
  while (1) {
    ...
    while ((nread = getline(&line, &llen, stream)) != -1){
      ...
      char* fn             = strtok(line, "|");        // lowry: 种子文件名
      char* bdr            = strtok(strtok(NULL,"|"),"\n");

      boundary = atoi(bdr);

      if (line_cnt == boundary && bflag == 0){
		bflag = 1;
        send(sock, "atten", 5, 0);
      }
      ...
      if (fflag == 0){
		...
		/* lowry: 如果接收到信号，则退出当前fuzzing */
		if (bflag == 1 && read(sock, buf, 5) > 0){
			bflag = 0;
			printf("recieve py sign: %s\n", buf);
			free(line);
    		fclose(stream);
			copy_file("weight_info", weight_file);
			stream = fopen(weight_file, "r");         // lowry: 将文件读入数据流
			line = NULL;                              // lowry: 用于存储文件中每行的信息
			llen = 0;                                
			nread;
			/* lowry: 文件读取失败 */
			if (stream == NULL){
